...
- P14d：`VisualMockFeatureProvider -> mock VisualRouterHead -> RouterOutput ->
  EvaluationInputAdapter` protocol smoke，输入仍用 P13b/P14b tiny fixture 与 P13d
  `ExpertBatch` 数值参考，输出只在内存检查 summary/rows。
- P14d 可选覆盖 legacy embedding path wrapper 的最小 shape/order smoke，但不接真实
  Hugging Face ViT、不访问 `/data2`。
- P14e：legacy Visual MLP eval-only adapter audit 或 smoke，重点检查
...
3. P15：branch-specific small entrypoint decision，根据 P13d/P13e/P14a/P14b/P14c/P14d
