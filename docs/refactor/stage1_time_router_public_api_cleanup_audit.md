...
把明显的 smoke-only mock 从 package `__all__` 中移出，但保留兼容属性，避免破坏
既有 smoke 和 Visual small 默认路径。
...
| `time_router.features.visual_mock` | C. smoke-only scaffold | `VisualMockFeatureProvider`、`DeterministicVisualEncoderStub` | 从 `time_router.features.__all__` 移出；保留 `time_router.features.VisualMockFeatureProvider` 属性以兼容 P14/P15/P16 smoke。P18b/P18c 应迁到 `tests/helpers` 或 legacy smoke helper。 |
...
1. 将 `VisualMockFeatureProvider` 和 `DeterministicVisualEncoderStub` 迁到
   `tests/helpers` 或 `tests/smoke/helpers`，再把旧 smoke / Visual small 默认路径改为显式
   测试 helper import。迁移前必须先保留兼容层，避免 P15/P16 回归 smoke 同时断裂。
...
- `VisualMockFeatureProvider`、`DeterministicVisualEncoderStub` 不在
  `time_router.features.__all__`。
- 兼容期仍保留上述 smoke-only 属性，旧 smoke 不需要立即改 import。
