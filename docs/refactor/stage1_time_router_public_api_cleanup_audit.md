...
4. `visual_chain.py` 和 P19a `visual_chain_runner.py` 应保留到 ViT provider 迁移后再判断拆分方式。
   真实实现出现前，协议文件只表达 contract，runner 只表达编排和 lineage，不应承诺具体 RevIN、
   pseudo image、resize、HF processor、CLS/mean pooling 或 scaler fit 行为。
