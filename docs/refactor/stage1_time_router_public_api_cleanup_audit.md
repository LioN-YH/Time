...
P19a 在此基础上新增 `VisualFeatureChainRunner` 作为真实 Visual feature chain dry-run
skeleton 的编排器；它仍属于 Stage 1 migration bridge，只组合 P16f protocols，不加载真实
ViT，不引入 fake encoder 到 public core，也不改变 P17 precomputed eval 默认路径。
P19b 继续新增 guarded `VisualVitEncoderProvider` 和 `visual_vit_guard`，仍属于 Stage 1
migration bridge：默认 import 不导入 transformers，真实 ViT model / processor path 必须由
调用方显式传入并通过 Runtime guard，不迁移训练入口。
...
| `time_router.features.visual_chain_runner` | B. migration bridge / dry-run orchestrator | `VisualFeatureChainRunner`、`VisualFeatureChainResult`、`CHAIN_LINEAGE` | P19a 后保持 public `__all__`。Runner 只串联 P16f protocols、校验 sample_key 保序并合并 FeatureBatch lineage；不加载真实 ViT、不导入 transformers、不读取 checkpoint、不接正式训练入口。 |
| `time_router.features.visual_vit_encoder` | B. migration bridge / guarded ViT encoder adapter | `VisualVitEncoderProvider`、`build_visual_vit_encoder_provider` | P19b 后保持 public `__all__`。Provider 实现 `VisualEncoderProvider.encode`，输出 `VisualEmbeddingBatch`；模块 import 阶段不导入 transformers，真实 `AutoImageProcessor` / `ViTModel` 只在显式构造函数内部 lazy import；不接收 run_dir、不搜索模型路径、不训练。 |
| `time_router.runtime.visual_vit_guard` | B. migration bridge | `VisualVitModelPathPolicy`、`authorize_visual_vit_model_paths`、`is_fixture_or_tempfile_visual_vit_artifact` | P19b 后保持 public `__all__`。只做显式 ViT model / processor path guard；默认允许 fixture/tmp，非 fixture/tmp 需 `allow_real_vit`，`/data2` 需额外 `allow_external_vit_path`；不读取文件、不导入 transformers。 |
...
4. `visual_chain.py`、P19a `visual_chain_runner.py` 和 P19b `visual_vit_encoder.py` 应保留到
   真实 small/manual entrypoint 迁移后再判断拆分方式。协议文件只表达 contract，runner 只表达
   编排和 lineage，ViT provider 只表达 guarded encoder adapter，不应承诺具体 RevIN、
   pseudo image、resize、CLS/mean pooling、scaler fit 或训练入口行为。
