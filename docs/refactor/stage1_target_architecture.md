...
当前默认仍可由 scripts / exp_scripts / configs 触发既有 streaming entrypoint，但长期 contract 以 `docs/refactor/stage1_provider_interface.md` 定义的 provider chain 为准。`frozen ViT` 和 `17维 TimeFuse feature cache` 是当前实现选项，不是 interface 的唯一合法形态。P5d adapter 边界审查见 `docs/refactor/provider_adapter_boundary.md`：第一批实现应优先规划入口迁移，再包装 `PredictionBatchReader` 为最小 `PredictionCacheExpertProvider`；TimeFuse feature cache provider 可作为第二批 feature-only adapter，Visual online ViT provider 因运行时复杂度更高不作为第一批最小实现。P5e/P10h 入口迁移计划见 `docs/refactor/stage1_entrypoint_migration_plan.md`：当前主视角已调整为 `SampleManifest + SplitStrategy -> ExpertProvider / prediction backend -> SupervisionProvider -> FeatureProvider -> RouterHead -> EvaluationInputAdapter / Evaluator -> Runtime / artifact writer` 的 canonical dataflow；新 adapter 先由 smoke 使用，正式 streaming 入口后续围绕 shared manifest/split、prediction backend、supervision、evaluation 和 artifact contract 小步接入，同时保留 Visual / TimeFuse 的分支 feature、head 和 objective。P11a canonical run artifact schema 见 `docs/refactor/stage1_canonical_run_artifact_schema.md`：future canonical `run_dir` 推荐收束为 `run_metadata.json`、`run_status.json`、`inputs/`、`indexes/`、`predictions/`、`evaluation/`、`checkpoints/` 和 `logs/`；`run_dir` 属于 Runtime，不属于 Provider；旧版 `96_48_S` full-scale 输出只作为 sanity reference，不作为新 schema 的兼容目标。P5f launcher architecture 见 `docs/refactor/launcher_architecture.md`：未来实验启动层采用 `exp_scripts/*.sh -> scripts/*.py -> time_router runtime/protocol/provider/head/evaluator`，Bash 只做资源、config、run_dir 和后台策略编排，不承载训练逻辑。P6a 已新增 `time_router.experts.PredictionCacheExpertProvider` 的 smoke-only adapter，详细见 `docs/refactor/prediction_cache_expert_provider.md`；它只包装 `PredictionBatchReader` 为 `ExpertBatch`，尚未接入正式训练入口。P6a.5 专家系统边界审计见 `docs/refactor/expert_system_boundary_review.md`：`ExpertProvider / ExpertBatch` 是 Time framework 长期专家系统边界，固定五专家顺序只是当前 Stage 1 canonical experiment 契约，prediction cache 只是实现选项。P6b 已新增 `time_router.evaluation.EvaluationInputAdapter` smoke-only adapter，详细见 `docs/refactor/evaluation_input_adapter.md`；它只消费 `ExpertBatch + RouterOutput.weights` 或显式 fusion weights 并调用 evaluation public API 复算内存 summary/rows，不读取 prediction cache、oracle/TSF 或正式输出目录。P7a 已新增 `time_router.features.TimeFuseFeatureCacheProvider` smoke-only adapter，详细见 `docs/refactor/timefuse_feature_cache_provider.md`；它只把显式 feature CSV 的 TimeFuse 17 维 feature batch 包装为 `FeatureBatch`，不读取 prediction cache、oracle/TSF、`y_true` 或 expert error，不做 scaler fit，不创建输出目录。P7b 已新增 `time_router.models.TimeFuseLinearSoftmaxHead` smoke-only adapter，详细见 `docs/refactor/timefuse_linear_head.md`；它只把 `FeatureBatch.features` 映射为 `RouterOutput(logits, weights)`，不训练、不读取 cache、不写运行产物。P7c 已新增 TimeFuse protocol chain smoke，详细见 `docs/refactor/timefuse_protocol_chain_smoke.md`；它串联 ExpertProvider、FeatureProvider、LinearSoftmaxHead 和 EvaluationInputAdapter，验证协议对象可组合，但不迁移正式入口。P8d baseline parity 审计见 `docs/refactor/timefuse_baseline_parity_review.md`：当前分支应被称为 TimeFuse-style / TimeFuse-inspired / adapted TimeFuse-style baseline，不应声称完全复现原版 TimeFuse 或与原论文数值一一复现比较。P9a Visual Router 入口 adapter 插入审计见 `docs/refactor/visual_router_entrypoint_adapter_insertion_audit.md`：Visual Router 第一批接入应只做 evaluation 阶段旁路校验和 ExpertBatch 对齐规划，不迁移 Quito history window、pseudo image、ViT provider、router head、loss 或正式输出 schema。P9e gap audit 见 `docs/refactor/visual_router_prediction_cache_provider_gap_audit.md`：`PredictionCacheExpertProvider` 已具备 `ExpertBatch` 包装能力，但 Visual Router 正式 SQLite path 仍承担 required sample_key 推导、manifest chunk scan、SQLite 子集索引、batch query、runtime index artifact metadata 和 `fusion_huber_kl` expert error 计算；短期继续保留 SQLite path，只做 batch 后 ExpertBatch 旁路校验。P10a shared SQLite backend 审计见 `docs/refactor/shared_prediction_sqlite_backend_audit.md`：shared backend 只覆盖 prediction index prepare / fetch / packed loading，不接管 Visual/TimeFuse feature、loss、runtime artifact、launcher 或 oracle deployable feature 边界。P10d/P10e canonical sample/supervision boundary 见 `docs/refactor/stage1_canonical_sample_supervision_boundary.md`：长期由 `SampleManifest + SplitStrategy` 提供 sample source、split 和顺序，由 `SupervisionProvider` 提供 oracle/error 训练监督；labels CSV、feature CSV、oracle reader 和 prediction reader 的历史分工不再作为长期接口边界。P10e 已新增 smoke-only `SampleManifestRow`、`SampleManifest` 和 `SupervisionBatch` 协议骨架，用于锁定 ordered sample_keys 与 supervision shape 对齐，不代表正式 provider 或训练入口已接入。P11b canonical SampleManifest physical schema 见 `docs/refactor/stage1_sample_manifest_physical_schema.md`：`stage1_sample_manifest_v1` 固定最小物理字段为 `sample_key`、`split`、`config_name`、`dataset_name`、`item_id`、`channel_id`、`window_index`、`seq_len`、`pred_len` 和轻量 `lineage`；`stage1_split_summary_v1` 写入 `inputs/split_summary.json`；`run_dir/inputs/` 可保存 snapshot 或 reference，但两者都必须能恢复 ordered sample_keys。P14a Visual FeatureProvider 插入审计见 `docs/refactor/stage1_visual_feature_provider_insertion_audit.md`：未来 Visual `FeatureProvider` 最小输出应为 `FeatureBatch(sample_keys, features, feature_schema, extra)`，history window / pseudo image / 可选 frozen ViT forward 是候选 provider 边界；device、dtype、DataParallel、Hugging Face cache、latency、checkpoint/resume 和 run artifact 仍由 Runtime / encoder factory 显式管理；`FeatureBatch` 与 `ExpertBatch` 只通过 ordered `sample_keys` 对齐。
...
| SampleManifest / SplitStrategy | 提供 canonical sample source、split 和 ordered sample_keys | P10e 已有最小 `SampleManifestRow` / `SampleManifest` 协议骨架与 smoke；长期作为 Visual Router 与 TimeFuse 共同主索引；split 不再散落在 labels CSV、feature CSV、oracle reader 或 prediction reader |
...
| SupervisionProvider / OracleLabelProvider | 批量提供 `oracle_model`、`oracle_value`、`per_model_errors`、`model_columns` 和 `metric` | P10e 已有最小 `SupervisionBatch` 协议骨架与 shape smoke；长期从 ExpertProvider 与 FeatureProvider 中分离；只用于训练监督、诊断、baseline 和 upper-bound，不进入 deployable feature |
...
P10h 后，entrypoint migration plan 的阅读顺序应先看 canonical dataflow，再看 Visual /
TimeFuse 的分支实现差异。正式入口尚未整体迁移到该 dataflow；当前完成的是
Visual evaluation/training `ExpertBatch` bypass、Visual labels adapter smoke、TimeFuse protocol
chain smoke、TimeFuse sample/supervision adapter smoke 和 shared prediction SQLite backend smoke。
P11a 已冻结 future canonical run artifact schema；P11b 已冻结 canonical `SampleManifest`
physical schema、split summary schema 和 `run_dir/inputs/` snapshot/reference 保存方式。下一步应先
审计真实 full-scale Visual labels schema 与 TimeFuse feature/oracle schema，并在 P11c 设计最小
Runtime artifact writer 接入边界，最后准备 small / pressure / full-scale scripts。
