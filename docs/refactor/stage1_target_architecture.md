...
| FusionEvaluator adapter | 将 `ExpertBatch + RouterOutput` 或 `EvaluationInput` 适配到 evaluation public API | P6b 已完成 smoke-only adapter；只复算内存 summary/per-sample rows；不重新读取 prediction cache、manifest、packed npy、oracle/TSF，不创建 run_dir 或写文件 |
