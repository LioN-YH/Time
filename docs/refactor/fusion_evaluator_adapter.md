...
本文件记录 P6b 最小 FusionEvaluator adapter 的实现边界。该 adapter 位于 `time_router/evaluation/`，只负责把 P5c protocol objects 适配成 `time_router.evaluation` public API 可消费的内存对象，并复算当前 golden smoke 锁定的 summary 与 per-sample rows。
...
因此 hard top-1、raw soft fusion、MAE/MSE、selected counts、max weight、weight entropy 和 per-sample rows 口径继续由 P3a-P3d helper 决定，P6b 不新增 calibration、temperature、top-k、oracle regret 或正式 output schema。
...
该 smoke 使用 2026-06-14 的 4 sample packed golden fixture：

1. 构造 `PredictionCacheExpertProvider`。
2. 显式传入 golden sample_keys 得到 `ExpertBatch`。
3. 构造包含 golden weights 的 `RouterOutput`。
4. 通过 `FusionEvaluator` 构造 `EvaluationInput` 并复算 hard top-1 / raw soft result。
