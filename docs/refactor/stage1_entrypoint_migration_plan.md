...
- P16b 已完成 real Visual feature provider 边界审计，见
  ...
  interface。
- P16e 已完成 Visual feature architecture variant 边界收束，见
  `docs/refactor/stage1_visual_feature_architecture_variants.md`；进一步明确 P16a-P16d 不是把
  CSV/cache/scaler 或某一种 ViT pooling 固化为唯一方案，长期固定的是 ordered sample_keys、
  `FeatureProvider` / `FeatureTransform`、`FeatureBatch`、RouterHead adapter、Evaluator 和
  Runtime artifact writer 的交接契约；RevIN、resize、pseudo image、encoder choice、CLS /
  mean_patch / mean_pool pooling、scaler/normalizer、precompute embedding 和 cache 都应作为
  可替换 architecture variant 插槽接入。
- P16f 已新增 Visual feature chain protocol skeleton，见
  `docs/refactor/stage1_visual_feature_chain_protocol.md`；进一步把 Visual feature chain 拆成
  raw window、pre-image transform、pseudo image、resize/input policy、visual encoder、
  pooling 和 feature transform 插槽。该 protocol 只表达 ordered sample_keys、payload shape
  和轻量 lineage，不绑定真实 ViT、cache、checkpoint 或 run_dir。
...
- P16a 已新增 `LoadedTorchMLPRouterHeadAdapter`，只包装 Runtime 已加载的
  `torch.nn.Module`，消费 head-ready `float32 FeatureBatch` 与显式 `model_columns`，输出
  `RouterOutput(logits, weights)`；它不读取 checkpoint、不处理 scaler、不启动 ViT、不知道
  `run_dir`。
- P16g 已完成 legacy `VisualMLPRouter` checkpoint/signature 审计，见
  `docs/refactor/stage1_visual_legacy_mlp_checkpoint_signature_audit.md`；legacy constructor 为
  `input_dim, hidden_dim, output_dim, dropout`，forward 返回二维 logits；streaming checkpoint
  使用 `router_state_dict` payload，scaler/device/DataParallel key 处理仍属于 Runtime。
- P16h 已完成 legacy `VisualMLPRouter` loaded-module smoke，见
  `docs/refactor/stage1_visual_legacy_mlp_loaded_module_smoke.md`；新增
...
- P16d 已完成 loaded Visual FeatureScaler boundary smoke，见
  `docs/refactor/stage1_visual_feature_scaler.md`；新增
  `time_router/features/visual_scaler.py`、
  `tests/fixtures/stage1_visual_scaler_small/` 和
  `tests/smoke/stage1_visual_feature_scaler_smoke.py`，用 P13b manifest 的 ordered
  sample_keys 从 raw/pre-head visual fixture 构造 `FeatureBatch`，再由
  `LoadedFeatureScaler` 使用显式 loaded scaler JSON state 转为 head-ready
