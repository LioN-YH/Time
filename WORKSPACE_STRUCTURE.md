...
| `time_router/features/__init__.py` | 共享特征适配器子包入口 | 导出 P7a 最小 `TimeFuseFeatureCacheProvider`、P14b smoke-only `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub`、P16c `VisualPrecomputedFeatureProvider` 和 P16d `LoadedFeatureScaler`；当前只用于 smoke 和 provider/transform 边界验证，不接正式 TimeFuse fusor / Visual Router 入口 |
