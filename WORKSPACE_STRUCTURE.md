...
| `scripts/run_stage1_visual_eval_canonical.py` | Stage 1 P17a Visual canonical evaluation entrypoint thin slice | 显式 CLI 串联 P13b/P16c fixture 风格的 `SampleManifest -> VisualPrecomputedFeatureProvider -> optional LoadedFeatureScaler -> Runtime-loaded legacy VisualMLPRouter checkpoint payload -> LoadedTorchMLPRouterHeadAdapter -> ExpertBatch -> EvaluationInputAdapter -> Runtime artifact writer`，在 `--output-dir/--run-id` 下写 canonical run_dir；这是 evaluation 入口雏形，不是训练入口，不自动搜索 checkpoint/feature，不访问 `/data2`，不启动 ViT/transformers，不修改 Visual small / TimeFuse small / streaming 正式入口；metadata 明确记录 `entrypoint=visual_eval_canonical`、feature/scaler/checkpoint 和 `training_started=false` |
