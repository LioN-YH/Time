...
| `docs/refactor/evaluation_input_adapter.md` | Stage 1 P6b EvaluationInput adapter | 记录 `time_router.evaluation.EvaluationInputAdapter` 的 smoke-only adapter API、`ExpertBatch + RouterOutput.weights / explicit fusion weights -> EvaluationInput -> evaluation public API` 适配流程、纯内存 summary/rows 输出、明确不做范围和 smoke 验收；本身不迁移正式 Visual Router / TimeFuse fusor 入口 |
| `docs/refactor/fusion_evaluator_adapter.md` | Stage 1 P6b FusionEvaluator adapter | 记录 `time_router.evaluation.FusionEvaluator` 的 smoke-only adapter API、`ExpertBatch + RouterOutput -> EvaluationInput -> evaluation public API` 适配流程、纯内存 summary/rows 输出、明确不做范围和 smoke 验收；本身不迁移正式 Visual Router / TimeFuse fusor 入口 |
...
| `time_router/evaluation/__init__.py` | 共享评估子包入口 | 导出 P3a/P3b/P3c/P3d 最小 fusion/metrics/router weight diagnostics/summary/rows helper，以及 P6b `EvaluationInputAdapter` 和兼容 `FusionEvaluator` smoke-only adapter；当前不包含 calibration、正式报告 schema 或训练入口迁移 |
| `time_router/evaluation/evaluation_input_adapter.py` | Stage 1 P6b 最小 EvaluationInput adapter | 纯内存实现 `EvaluationInputAdapter` 和 `EvaluationInputAdapterResult`，将 `ExpertBatch + RouterOutput.weights` 或显式 fusion weights 包装为 `EvaluationInput`，再调用 `hard_top1_fusion`、`raw_soft_fusion`、`build_fusion_summary` 和 `build_per_sample_fusion_rows`；保持 sample/model 顺序，复用 `ExpertBatch.y_pred/y_true`，不读取 manifest、prediction cache、packed npy、oracle/TSF，不访问 `/data2`，不创建 run_dir，不写 status/metadata/CSV/JSON/Parquet |
| `time_router/evaluation/fusion_evaluator.py` | Stage 1 P6b 最小 FusionEvaluator adapter | 纯内存实现 `FusionEvaluator` 和 `FusionEvaluationResult`，将 `ExpertBatch + RouterOutput` 或显式 `EvaluationInput` 适配到 `hard_top1_fusion`、`raw_soft_fusion`、`build_fusion_summary` 和 `build_per_sample_fusion_rows`；保持 sample/model 顺序，复用 `ExpertBatch.y_pred/y_true` 和 `RouterOutput.weights`，不读取 manifest、prediction cache、packed npy、oracle/TSF，不访问 `/data2`，不创建 run_dir，不写 status/metadata/CSV/JSON/Parquet |
...
| `tests/smoke/stage1_fusion_evaluator_adapter_smoke.py` | Stage 1 P6b FusionEvaluator adapter smoke | 默认读取同一 4 sample packed golden fixture，先用 `PredictionCacheExpertProvider` 显式构造 `ExpertBatch`，再用 golden weights 构造 `RouterOutput` 并调用 `FusionEvaluator`；验证 `EvaluationInput` 保序、固定五专家顺序、summary golden 数值、per-sample rows 字段/数值、hard/raw-soft MAE/MSE、max_weight 和 weight_entropy；adapter 调用阶段阻断 `open`、`Path.open` 和 `np.load`，并检查 `run_outputs` 一层目录集合不变，证明不重读 prediction cache/oracle/TSF、不创建正式输出目录 |
