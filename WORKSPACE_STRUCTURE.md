...
| `docs/refactor/stage1_refactor_roadmap.md` | Stage 1 后续小步重构路线图，按 P0-P10h 及 P2.5/P3a-P3e/P6a.5 等中间小步拆分 architecture docs、prediction reader、oracle/TSF reader、metrics/fusion、router weight diagnostics、summary、per-sample rows、evaluation package 边界复核、logging/path/config、FeatureProvider、专家系统边界审计、入口迁移、shared prediction SQLite backend、prediction array IO boundary consolidation、canonical SampleManifest / supervision boundary、最小协议 smoke、Visual labels adapter smoke、TimeFuse feature/oracle adapter smoke 和 canonical dataflow alignment review | 每个迁移步骤前后都应运行对应 smoke 并写实验日志 |
