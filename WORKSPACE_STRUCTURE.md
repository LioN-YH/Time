...
| `experiment_summaries/visual_router_v2_round1/` | Visual Router V2 Round 1 轻量汇总结果目录 | 保存适合随代码仓库远程审阅的 CSV/JSON/Markdown 汇总文件，不包含 checkpoint、SQLite、逐样本 prediction CSV 或大规模 cache；当前包含 P2probe、P2b visual pooling、P2c aux-only、P2d concat、P2d frozen final test、P2d frozen final test extension、P2e FiLM 和 Round 1 总表，其中 `p2d_final_test/` 复制自 `/data2/syh/Time/run_outputs/2026-06-20_visual_router_v2_round1_final_test/` 与 final_test_only feature cache 的 summary/metadata，`p2d_final_test_extension/` 复制 extension comparison、delta summary、metadata 和中文摘要，`p2e_film/` 复制 FiLM selection/diagnostic/delta/strata/metadata 和中文摘要 |
