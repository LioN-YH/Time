...
| `time_router/features/__init__.py` | 共享特征适配器子包入口 | 导出 P7a 最小 `TimeFuseFeatureCacheProvider`、P16c `VisualPrecomputedFeatureProvider`、P16d `LoadedFeatureScaler` 和 P16f visual chain protocol skeleton 类型；P18b 后不再从 package 入口导入 P14b smoke-only `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub`，后两者归属 `tests.helpers.visual_smoke_providers`；当前只用于 provider/transform/protocol 边界验证，不接正式 TimeFuse fusor / Visual Router 入口 |
