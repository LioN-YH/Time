...
| `docs/refactor/stage1_visual_small_loaded_legacy_path.md` | Stage 1 P16j Visual small loaded legacy path | 记录 Visual-specific small entrypoint 的 integrated rehearsal：默认 mock path 保持 P15c 行为，显式 loaded path 使用 P16c precomputed feature、可选 P16d scaler、P16i tiny checkpoint payload helper strict load legacy `VisualMLPRouter`，再交给 P16a adapter 和 Runtime artifact writer；明确仍只使用 small fixture / tempfile tiny checkpoint，不读取真实 checkpoint、不访问 `/data2`、不启动 ViT、不迁移正式入口 |
| `docs/refactor/stage1_visual_small_loaded_path_artifact_parity.md` | Stage 1 P16k Visual small loaded path artifact parity | 记录 Visual small entrypoint 默认 mock path 与显式 loaded legacy path 的 artifact parity smoke；比较 canonical run_dir 共同结构、metadata/status schema、input refs、evaluation summary schema、prediction rows schema、model_columns、split 列和 sample_key 顺序；明确两条路径 metrics 数值可不同，loaded path 仍只是 small fixture rehearsal |
| `docs/refactor/stage1_visual_eval_canonical_entrypoint.md` | Stage 1 P17a Visual eval canonical entrypoint | 记录 `scripts/run_stage1_visual_eval_canonical.py` 的 evaluation-only thin slice：显式 CLI 串联 SampleManifest、precomputed Visual feature、可选 loaded scaler、Runtime-loaded legacy `VisualMLPRouter` checkpoint payload、P16a adapter、ExpertBatch、EvaluationInputAdapter 和 Runtime artifact writer；明确不自动搜索 checkpoint/feature、不访问 `/data2`、不启动 ViT/训练/full-scale、不修改 Visual small / TimeFuse small / streaming 正式入口 |
| `docs/refactor/stage1_visual_eval_real_checkpoint_guard.md` | Stage 1 P17b Visual eval real checkpoint guard | 记录 P17b 在 canonical eval entrypoint 上新增的显式 real-checkpoint dry-run 授权路径：默认仍只允许 fixture/tmp tiny checkpoint，非 fixture/tmp 需 `--allow-real-checkpoint`，`/data2` 还需 `--allow-external-checkpoint-path`；明确 helper 只做 path policy、不读 `/data2`、不启动 ViT/训练/full-scale |
| `docs/refactor/stage1_visual_eval_external_feature_guard.md` | Stage 1 P17c Visual eval external feature/scaler guard | 记录 P17c 在 canonical eval entrypoint 上新增的显式 external precomputed feature/scaler dry-run 授权路径：默认仍只允许 fixture/tmp feature CSV 与 scaler JSON，非 fixture/tmp 需 `--allow-external-feature-path` 或 `--allow-external-scaler-path`；metadata 记录 feature/scaler path policy、allow flags、`scaler_fit_performed=false`，明确 helper 只做 path policy、不读 `/data2`、不启动 ViT/训练/full-scale |
| `docs/refactor/stage1_visual_eval_real_artifact_manual_dryrun.md` | Stage 1 P17d Visual eval real-artifact manual dry-run | 记录 P17d 在 canonical eval entrypoint 上新增的 manual real-artifact dry-run 口径：用户显式提供真实 legacy `VisualMLPRouter` checkpoint payload、真实 precomputed visual feature CSV 和可选 scaler state JSON 时，通过 `--manual-real-artifact-dryrun` 或 smoke 环境变量路径运行 evaluation-only dry-run；metadata/stdout 记录 `manual_real_artifact_dryrun`、真实 checkpoint、external feature/scaler allow 状态，明确默认测试 skip、不搜索 `/data2`、不启动 ViT/训练/full-scale |
| `docs/refactor/stage1_time_router_public_api_cleanup_audit.md` | Stage 1 P18a time_router public API cleanup audit | 审计 `time_router` 当前 protocols/runtime/features/models/evaluation/experts public exports、P17 canonical eval 入口依赖和 smoke-only helper/mock；按长期 canonical core、Stage 1 migration bridge、smoke-only scaffold 分类；P18b 后补充记录 `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub` 已迁到 `tests.helpers.visual_smoke_providers`，`time_router.features.visual_mock` 仅保留 P18c 前兼容 wrapper |
| `docs/refactor/stage1_time_router_smoke_scaffold_cleanup.md` | Stage 1 P18b time_router smoke scaffold cleanup | 记录 P18b 如何把 visual smoke-only `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub` 实现迁到 `tests/helpers/visual_smoke_providers.py`，更新 smoke 与 Visual small rehearsal import，收紧 `time_router.features` package 入口，并保留旧 `time_router.features.visual_mock` 子模块作为 P18c 前兼容层；明确不迁移 ViT provider、不迁移训练入口、不访问 `/data2` |
| `docs/refactor/stage1_visual_feature_chain_dryrun_skeleton.md` | Stage 1 P19a VisualFeatureChain dry-run skeleton | 记录 `VisualFeatureChainRunner` 的协议组件编排、sample_key 保序检查、FeatureBatch lineage schema、显式 raw window fixture、P19a smoke 验收命令和不加载真实 ViT / 不导入 transformers / 不迁移训练入口的边界 |
...
| `scripts/run_stage1_visual_small.py` | Stage 1 P15c/P16j Visual-specific small canonical entrypoint | 默认使用 P13b real-derived small manifest/expert JSON 和 P14b Visual mock history window fixture，串联 `SampleManifest -> VisualMockFeatureProvider / FeatureBatch -> ExpertBatch -> script-local smoke-only MLP adapter / RouterOutput -> EvaluationInputAdapter -> Runtime artifact writer`，在 `--output-dir/--run-id` 下写 canonical run_dir；P16j 新增显式 loaded legacy path，可通过 `--feature-source precomputed`、`--scaler-state-json`、`--use-loaded-legacy-mlp` 和 `--router-checkpoint-payload` 串联 P16c/P16d/P16i/P16a；仍仅服务 Visual small rehearsal，不访问 `/data2`、不启动训练/pressure/full-scale、不读取真实 checkpoint、不启动 ViT embedding、不修改 generic 或 TimeFuse small CLI、不迁移正式 Visual Router 入口 |
| `scripts/run_stage1_visual_eval_canonical.py` | Stage 1 P17a/P17b/P17c/P17d Visual canonical evaluation entrypoint thin slice | 显式 CLI 串联 P13b/P16c fixture 风格或显式授权 external precomputed artifact 的 `SampleManifest -> VisualPrecomputedFeatureProvider -> optional LoadedFeatureScaler -> Runtime-loaded legacy VisualMLPRouter checkpoint payload -> LoadedTorchMLPRouterHeadAdapter -> ExpertBatch -> EvaluationInputAdapter -> Runtime artifact writer`，在 `--output-dir/--run-id` 下写 canonical run_dir；这是 evaluation 入口雏形，不是训练入口，不自动搜索 checkpoint/feature/scaler，不启动 ViT/transformers，不修改 Visual small / TimeFuse small / streaming 正式入口；P17b 新增 checkpoint allow flags，P17c 新增 `--allow-external-feature-path`、`--allow-external-scaler-path`、`--feature-path-label` 和 `--scaler-path-label`，P17d 新增 `--manual-real-artifact-dryrun` metadata/stdout 口径；默认仍只允许 fixture/tmp feature/scaler/checkpoint，`/data2` artifact 只有显式授权后才允许进入文件读取阶段；metadata 明确记录 `entrypoint=visual_eval_canonical`、`manual_real_artifact_dryrun`、feature/scaler/checkpoint/path policy、`scaler_fit_performed=false` 和 `training_started=false` |
...
| `time_router/features/__init__.py` | 共享特征适配器子包入口 | 导出 P7a 最小 `TimeFuseFeatureCacheProvider`、P16c `VisualPrecomputedFeatureProvider`、P16d `LoadedFeatureScaler`、P16f visual chain protocol skeleton 类型和 P19a `VisualFeatureChainRunner`；P18b 后不再从 package 入口导入 P14b smoke-only `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub`，后两者归属 `tests.helpers.visual_smoke_providers`；当前只用于 provider/transform/protocol/runner 边界验证，不接正式 TimeFuse fusor / Visual Router 入口 |
...
| `time_router/features/visual_mock.py` | Stage 1 P18b visual mock compatibility wrapper | P18b 后只 re-export `tests.helpers.visual_smoke_providers` 中的 `DeterministicVisualEncoderStub` 和 `VisualMockFeatureProvider`，用于兼容 P18c 前直接导入旧子模块的历史代码；不由 `time_router.features` package 入口导入，不进入 public `__all__` |
...
| `time_router/features/visual_chain_runner.py` | Stage 1 P19a VisualFeatureChain dry-run orchestrator | 提供 `VisualFeatureChainRunner`、`VisualFeatureChainResult` 和 `CHAIN_LINEAGE`；只组合 P16f protocols、执行每层 sample_key 保序检查，并把 raw_window/pseudo_image/resize/encoder/pooling/feature_transform lineage 合并进 canonical `FeatureBatch`；不加载真实 ViT、不导入 transformers、不读取 checkpoint、不接正式 streaming 训练入口 |
...
| `time_router/runtime/__init__.py` | Stage 1 Runtime 子包入口 | 导出 canonical artifact writer、P16i Visual MLP checkpoint payload helper 和 P17b Visual eval checkpoint path guard public API；入口只聚合 Runtime 层 API，不读取配置、不自动搜索 checkpoint、不启动训练 |
...
| `time_router/runtime/visual_mlp_checkpoint.py` | Stage 1 P16i legacy VisualMLPRouter checkpoint payload helper | Runtime-side 从显式 checkpoint path 读取 payload、提取 `router_state_dict`、清理 DataParallel `module.` 前缀并 strict load 到已构造 legacy VisualMLPRouter；不做 run_dir discovery、不拼默认 checkpoint、不访问外部固定路径、不启动 ViT、不把 path 下沉到 adapter |
| `time_router/runtime/visual_eval_checkpoint_guard.py` | Stage 1 P17b Visual eval checkpoint path guard | 对显式 checkpoint payload path 执行授权策略：默认只允许 `tests/fixtures` 或 `/tmp` tiny payload，非 fixture/tmp 需 `--allow-real-checkpoint`，`/data2` 需额外 `--allow-external-checkpoint-path`；helper 只做 path policy，不检查文件存在、不调用 `torch.load`、不读取 `/data2` 内容、不自动搜索 checkpoint |

### 2.6 `tests/helpers/`

| 路径 | 层级角色 | 功能 |
| --- | --- | --- |
| `tests/helpers/__init__.py` | tests helper package 入口 | 集中 tests/smoke 专用辅助对象，避免 dummy/mock/stub 长期暴露在 `time_router` 正式 public API 边界中 |
| `tests/helpers/visual_smoke_providers.py` | Stage 1 P18b visual smoke provider helper | 承接 P14b/P15/P16 smoke-only `DeterministicVisualEncoderStub` 和 `VisualMockFeatureProvider`；只消费调用方显式传入的内存 `sample_key -> history_window_x` 映射并输出 8 维 `float32 FeatureBatch`，不读取 prediction/oracle/run_dir/checkpoint，不加载真实 ViT，不访问 `/data2` |
...
| `tests/smoke/stage1_visual_small_entrypoint_loaded_legacy_path_smoke.py` | Stage 1 P16j Visual small loaded legacy path smoke | 使用 tempfile 创建 tiny legacy VisualMLPRouter checkpoint payload，通过 subprocess 调用 `scripts/run_stage1_visual_small.py --feature-source precomputed --scaler-state-json ... --use-loaded-legacy-mlp --router-checkpoint-payload ...`；验证 canonical run_dir、run_status completed、metadata 中 loaded legacy/precomputed/scaler/非正式迁移边界、evaluation summary hard/raw-soft MAE/MSE、prediction_rows sample_key 保序、model_columns 与 ExpertBatch 对齐，stdout/stderr 未出现 `/data2`、ViT/transformers 或正式 streaming 入口 |
| `tests/smoke/stage1_visual_small_loaded_path_artifact_parity_smoke.py` | Stage 1 P16k Visual small loaded path artifact parity smoke | 使用 tempfile output root 同时运行 `scripts/run_stage1_visual_small.py` 默认 mock path 和显式 loaded legacy path；loaded path 使用 P16c precomputed fixture 与 tempfile tiny checkpoint payload、no-scaler 口径；比较两边 canonical run_dir 共同 artifact、metadata/status/evaluation/prediction rows schema、row count、sample_key 顺序、split 列、model_columns 和有限指标字段，确认 stdout/stderr 未出现 `/data2`、ViT 或正式 streaming 入口 |
| `tests/smoke/stage1_visual_eval_canonical_thin_slice_smoke.py` | Stage 1 P17a Visual eval canonical entrypoint smoke | 使用 P13b manifest/expert JSON、P16c precomputed visual feature fixture 和 tempfile tiny legacy `VisualMLPRouter` checkpoint payload，通过 subprocess 调用 `scripts/run_stage1_visual_eval_canonical.py`；验证 canonical run_dir、`run_status=completed`、metadata 中 `entrypoint=visual_eval_canonical`、precomputed feature、loaded legacy、no-scaler、非训练/非 ViT 边界，evaluation summary hard/raw-soft MAE/MSE、selected_counts/model_columns 和 prediction_rows sample_key 保序，并确认 stdout/stderr 未出现 `/data2`、ViT/transformers 或正式 streaming 入口 |
| `tests/smoke/stage1_visual_eval_canonical_real_checkpoint_guard_smoke.py` | Stage 1 P17b Visual eval real-checkpoint guard smoke | 覆盖 canonical eval 默认 tempfile tiny checkpoint 成功且 `loads_real_checkpoint=false`、仓库根目录非 fixture/tmp checkpoint 未授权时 fail-fast 并提示 `--allow-real-checkpoint`、`/data2` checkpoint path guard 只调用 helper 不创建/读取文件且需要 `--allow-real-checkpoint` + `--allow-external-checkpoint-path` 双重授权；可选环境变量 manual real-checkpoint dry-run 默认跳过 |
| `tests/smoke/stage1_visual_eval_canonical_external_feature_guard_smoke.py` | Stage 1 P17c Visual eval external feature/scaler guard smoke | 覆盖 canonical eval 默认 fixture feature CSV + no scaler 成功且 `allow_external_feature_path=false`、仓库根目录非 fixture/tmp feature CSV 未授权 fail-fast、授权后 metadata `feature_path_policy=explicit_external_feature_authorized`、非 fixture/tmp scaler JSON 未授权失败/授权后成功且 `scaler_enabled=true` 与 `scaler_fit_performed=false`、`/data2` feature/scaler path guard 只调用 helper 不创建/读取文件；可选环境变量 manual external feature dry-run 默认跳过 |
| `tests/smoke/stage1_visual_eval_canonical_manual_real_artifact_contract_smoke.py` | Stage 1 P17d Visual eval real-artifact manual dry-run contract smoke | 覆盖 manual 环境变量不完整时按预期 skip、仓库受控临时目录 synthetic real-artifact checkpoint + external feature CSV + external scaler JSON 显式授权后成功写 canonical run_dir、metadata `manual_real_artifact_dryrun=true` 与真实 checkpoint/external feature/scaler/no ViT/no training 边界、finite metrics、prediction_rows sample_key 保序，以及 checkpoint input_dim mismatch 和 feature CSV 缺 sample_key 两类 contract fail-fast；默认不读取真实 `/data2` 内容、不启动 ViT/训练/full-scale |
| `tests/smoke/stage1_time_router_public_api_boundary_smoke.py` | Stage 1 P18a/P18b time_router public API boundary smoke | 只做 import boundary 检查：验证 `time_router` 核心子包 import 成功，protocols/runtime/features/models/evaluation/experts 中 canonical core 与 P17 migration bridge public `__all__` 可用，P18b 后 `VisualMockFeatureProvider` 和 `DeterministicVisualEncoderStub` 既不进入 `time_router.features.__all__` 也不作为 package 入口属性保留；import P17 canonical eval entrypoint 文件后确认未加载 `transformers`，不访问 `/data2`、不启动 ViT/训练/full-scale |
| `tests/smoke/stage1_time_router_public_api_smoke_scaffold_cleanup_smoke.py` | Stage 1 P18b time_router smoke scaffold cleanup smoke | 验证 `VisualPrecomputedFeatureProvider` / `LoadedFeatureScaler` 仍可从 `time_router.features` public API 导入，`VisualMockFeatureProvider` / `DeterministicVisualEncoderStub` 不在 `features.__all__` 或 package 入口属性中，`tests.helpers.visual_smoke_providers` 可承接 mock/stub 能力，旧 `time_router.features.visual_mock` 子模块兼容层指向 helper 实现；import 阶段不加载 `transformers`，不访问 `/data2`、不启动 ViT/训练/full-scale |
| `tests/smoke/stage1_visual_feature_chain_dryrun_skeleton_smoke.py` | Stage 1 P19a VisualFeatureChain dry-run skeleton smoke | 使用 P13b manifest ordered sample_keys、显式 raw window JSON fixture、smoke-local no-transformers fake encoder、pseudo image/resize/pooling/transform 组件和内存小型 MLP，验证 `VisualFeatureChainRunner -> FeatureBatch -> LoadedTorchMLPRouterHeadAdapter -> EvaluationInputAdapter` 链路；覆盖 sample_key 保序、二维 `float32` finite features、schema/metadata lineage、未导入 transformers、未访问 `/data2`、未修改正式 streaming 训练入口 |
...
| `tests/fixtures/stage1_visual_feature_chain_dryrun/` | Stage 1 P19a VisualFeatureChain dry-run raw window fixture | 保存 `raw_windows.json` 和 README；4 个 sample_key 来自 P13b manifest，每个 raw window 长度 6，仅用于 P19a runner smoke 的显式 raw window provider、shape、保序和 lineage 验证；不来自 `/data2`，不包含真实 ViT embedding |
