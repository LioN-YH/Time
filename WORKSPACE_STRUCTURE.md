...
| `tests/smoke/stage1_prediction_sqlite_backend_smoke.py` | Stage 1 P10b shared prediction SQLite backend smoke | 在 tempfile 下构造 4 sample × 5 model 的 packed_npy_v1 manifest 和数组，验证 `build_prediction_sqlite_backend(...)`、SQLite metadata、`fetch_records(...)`、`records_to_ordered_rows(...)`、grouped packed loading、row index lineage、shape、默认缺失报错和 `allow_missing=True` missing report；不访问 `/data2`、不运行正式入口、不写正式输出 |
