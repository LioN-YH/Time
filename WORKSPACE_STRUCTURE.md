...
| `scripts/run_stage1_visual_eval_canonical.py` | Stage 1 P17a/P17b Visual canonical evaluation entrypoint thin slice | 显式 CLI 串联 P13b/P16c fixture 风格的 `SampleManifest -> VisualPrecomputedFeatureProvider -> optional LoadedFeatureScaler -> Runtime-loaded legacy VisualMLPRouter checkpoint payload -> LoadedTorchMLPRouterHeadAdapter -> ExpertBatch -> EvaluationInputAdapter -> Runtime artifact writer`，在 `--output-dir/--run-id` 下写 canonical run_dir；这是 evaluation 入口雏形，不是训练入口，不自动搜索 checkpoint/feature，不启动 ViT/transformers，不修改 Visual small / TimeFuse small / streaming 正式入口；P17b 新增 `--allow-real-checkpoint`、`--allow-external-checkpoint-path` 与 `--checkpoint-path-label`，默认仍只允许 fixture/tmp tiny checkpoint，`/data2` checkpoint 只有双重授权后才允许进入文件读取阶段；metadata 明确记录 `entrypoint=visual_eval_canonical`、feature/scaler/checkpoint/path policy 和 `training_started=false` |
