...
| `docs/refactor/stage1_time_router_public_api_cleanup_audit.md` | Stage 1 P18a time_router public API cleanup audit | 审计 `time_router` 当前 protocols/runtime/features/models/evaluation/experts public exports、P17 canonical eval 入口依赖和 smoke-only helper/mock；按长期 canonical core、Stage 1 migration bridge、smoke-only scaffold 分类；记录 `VisualMockFeatureProvider` / `DeterministicVisualEncoderStub` 从 `features.__all__` 移出但保留兼容属性的口径，以及 P18b/P18c 后续迁出/合并建议 |
...
| `tests/smoke/stage1_time_router_public_api_boundary_smoke.py` | Stage 1 P18a time_router public API boundary smoke | 只做 import boundary 检查：验证 `time_router` 核心子包 import 成功，protocols/runtime/features/models/evaluation/experts 中 canonical core 与 P17 migration bridge public `__all__` 可用，`VisualMockFeatureProvider` 和 `DeterministicVisualEncoderStub` 未进入 `time_router.features.__all__` 但兼容属性仍存在；import P17 canonical eval entrypoint 文件后确认未加载 `transformers`，不访问 `/data2`、不启动 ViT/训练/full-scale |
