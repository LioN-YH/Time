...
| `docs/refactor/stage1_refactor_roadmap.md` | Stage 1 后续小步重构路线图，按 P0-P10e 及 P2.5/P3a-P3e/P6a.5 等中间小步拆分 architecture docs、prediction reader、oracle/TSF reader、metrics/fusion、router weight diagnostics、summary、per-sample rows、evaluation package 边界复核、logging/path/config、FeatureProvider、专家系统边界审计、入口迁移、shared prediction SQLite backend、prediction array IO boundary consolidation、canonical SampleManifest / supervision boundary 和最小协议 smoke | 每个迁移步骤前后都应运行对应 smoke 并写实验日志 |
...
| `docs/refactor/stage1_canonical_sample_supervision_boundary.md` | Stage 1 P10d/P10e canonical SampleManifest 与 supervision boundary | 定义 Visual Router 与 TimeFuse-style fusor 可共用的 `SampleManifest`、`SplitStrategy` 和 `SupervisionProvider` 边界；明确 canonical manifest 字段、split 主索引、ExpertProvider 与 supervision 的区别、oracle/error 只用于训练监督/诊断/baseline/upper-bound、不进入 deployable FeatureProvider，并记录 P10e 已新增最小 `SampleManifestRow`、`SampleManifest`、`SupervisionBatch` 协议骨架与 smoke；不改正式入口、不新增正式 provider |
