...
| `experiment_summaries/visual_router_v2_round2/` | Visual Router V2 Round 2 轻量协议与 summary 目录 | 当前包含 `round2_small_screening_protocol.md`、`small_samples/`、`layout_imageization_smoke/` 和 `layout_screening/`；记录 view layout small screening 样本协议、layout candidate registry、top3fold 复用审计、Round2b layout imageization smoke 的 CSV/JSON/Markdown 和少量 debug thumbnails，以及 Round2c layout screening 的轻量 comparison/metadata/summary；`layout_screening/` 复制自正式输出 `/data2/syh/Time/run_outputs/2026-06-21_visual_router_v2_round2_layout_screening/`，best layout 为 `spatial_panel_3view`，selection/test_small raw-soft MAE 分别为 0.310385/0.398598；不包含 feature cache、checkpoint、SQLite、逐样本 prediction CSV、ViT embedding 或大规模伪图像 tensor |
...
| `visual_router_experiments/stage1_vali_test_router/build_visual_router_v2_round2_layout_features.py` | Visual Router V2 Round 2c layout feature cache builder | 从 `round2_small_sample_manifest.csv` 读取四个 frozen small sample set，按单 layout worker 生成 `features/<layout>/<sample_set>/shard_*.npz`，每个 shard 保存 `sample_key`、`order_index`、`layout_name`、`sample_set`、`cls_embedding`、`mean_patch_embedding` 和 `revin_aux`；layout 图像化通过 `round2_layout_registry.py`，ViT 输入使用 Round1 checkpoint 的 frozen encoder normalization，主路径不保存 pseudo image tensor，不读取专家 prediction/oracle label，不训练 router/encoder；`--aggregate-only` 单独生成 unified manifest、cache size、latency、metadata 和中文 summary |
| `visual_router_experiments/stage1_vali_test_router/train_visual_router_v2_round2_layout_film.py` | Visual Router V2 Round 2c 固定 FiLM backend screening 入口 | 对每个 layout 单独训练 Round1 best `film_mean_patch_aux` 风格 head：`mean_patch_embedding` 为 base visual input，`revin_aux` 只通过 FiLM gamma/beta 调制 hidden representation；`round2_train_small` 训练，`round2_selection_small` 选择，`round2_diagnostic_balanced_small` 诊断，`round2_test_small` frozen screening；支持 `--run-single`、`--build-index-only`、`--aggregate-only`，单任务只写独立 task 子目录，统一 comparison/summary 只由 aggregation 写出 |
