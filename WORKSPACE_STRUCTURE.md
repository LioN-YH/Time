...
| `scripts/run_stage1_visual_eval_canonical.py` | Stage 1 P17a/P17b/P17c Visual canonical evaluation entrypoint thin slice | 显式 CLI 串联 P13b/P16c fixture 风格或显式授权 external precomputed artifact 的 `SampleManifest -> VisualPrecomputedFeatureProvider -> optional LoadedFeatureScaler -> Runtime-loaded legacy VisualMLPRouter checkpoint payload -> LoadedTorchMLPRouterHeadAdapter -> ExpertBatch -> EvaluationInputAdapter -> Runtime artifact writer`，在 `--output-dir/--run-id` 下写 canonical run_dir；这是 evaluation 入口雏形，不是训练入口，不自动搜索 checkpoint/feature/scaler，不启动 ViT/transformers，不修改 Visual small / TimeFuse small / streaming 正式入口；P17b 新增 checkpoint allow flags，P17c 新增 `--allow-external-feature-path`、`--allow-external-scaler-path`、`--feature-path-label` 和 `--scaler-path-label`，默认仍只允许 fixture/tmp feature/scaler/checkpoint，`/data2` artifact 只有显式授权后才允许进入文件读取阶段；metadata 明确记录 `entrypoint=visual_eval_canonical`、feature/scaler/checkpoint/path policy、`scaler_fit_performed=false` 和 `training_started=false` |
