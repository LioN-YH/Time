...
| `visual_router_v2_panel_pooling.py` | Round2 `spatial_panel_3view` panel-wise pooling 共享模块；记录 224x224 / 16x16 ViT patch grid 的 line/fold/FFT panel region mapping，并从 `last_hidden_state` 输出 global mean、panel mean concat、global+panel mean 和 panel variance |
| `probe_visual_router_v2_panel_pooling.py` | Round2 panel-wise pooling architecture probe / small feature smoke；layout 固定为 `spatial_panel_3view`，只保存 pooled features 与 `revin_aux`，不保存 pseudo image tensor，不读取 oracle/prediction，不启动 full-scale |
| `train_visual_router_v2_panel_pooling_probe.py` | 可选 35k panel pooling FiLM probe 入口；消费 panel feature cache，比较 `film_mean_patch_aux`、`film_panel_mean_aux`、`film_global_panel_mean_aux`，aux 仍只通过 FiLM 注入，test_small 只做 frozen screening |
