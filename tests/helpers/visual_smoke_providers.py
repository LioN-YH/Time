"""
文件功能：
    提供 Stage 1 visual smoke 专用 FeatureProvider mock 与 deterministic encoder stub。

设计边界：
    本文件只服务 tests/smoke 和 Visual small rehearsal。这里的 provider 只消费
    调用方显式传入的 ordered sample_keys 和内存 history window source，并通过
    deterministic encoder stub 输出 FeatureBatch。它不读取 prediction cache、
    oracle/error、y_true、run_dir、status、checkpoint 或 `/data2`，也不加载真实
    Hugging Face ViT、不使用 GPU/DataParallel。
"""
...
class DeterministicVisualEncoderStub:
    ...
    关键约束：
        这是 smoke-only deterministic stub，不加载真实 ViT，不访问 GPU，
...
class VisualMockFeatureProvider:
    """
    类功能：
        Stage 1 visual smoke 最小 FeatureProvider mock。
