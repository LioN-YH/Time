...
关键约束：
    该 smoke 只允许 provider 阶段读取 golden fixture；进入
    EvaluationInputAdapter 后会阻断常见文件读取 API，验证 adapter 不重新读取
    prediction cache、不访问 oracle/TSF、不创建正式输出目录，也不写结果文件。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run Stage 1 EvaluationInput adapter smoke.")
    parser.add_argument(
        "--fixture-root",
        type=Path,
...
def snapshot_run_outputs() -> set[str]:
    """函数功能：记录 run_outputs 一层目录名，用于检查 adapter 不创建输出目录。"""
...
def forbid_adapter_file_reads() -> Iterator[None]:
    ...
    def fail_open(*_args: object, **_kwargs: object) -> object:
        raise AssertionError("EvaluationInputAdapter 阶段不应调用 open/path.open")

    def fail_np_load(*_args: object, **_kwargs: object) -> object:
        raise AssertionError("EvaluationInputAdapter 阶段不应调用 np.load 读取 prediction cache")
...
def assert_adapter_result(
    ...
    *,
    ...
    if result.evaluation_input.weights is not GOLDEN_WEIGHTS:
        raise AssertionError("EvaluationInput 必须复用 RouterOutput.weights 或显式 fusion weights")

    if result.extra.get("adapter_name") != "EvaluationInputAdapter":
        raise AssertionError(f"adapter extra 缺少 adapter_name：{result.extra}")
    if result.extra.get("weights_source") != expected_weights_source:
        raise AssertionError(f"adapter extra weights_source 漂移：{result.extra}")
    if "row_index_metadata" not in result.extra or "expert_extra" not in result.extra:
        raise AssertionError(f"adapter extra 应保留 ExpertBatch 轻量 lineage：{result.extra}")
...
def run_smoke(fixture_root: Path, *, atol: float) -> None:
    """函数功能：执行 EvaluationInput adapter 最小 contract smoke。"""
    fixture_root = fixture_root.resolve()
    print(f"开始 Stage 1 EvaluationInput adapter smoke：fixture_root={fixture_root}")
    ...
    with forbid_adapter_file_reads():
        router_result = adapter.evaluate(expert_batch=expert_batch, router_output=router_output)
        explicit_result = adapter.evaluate(expert_batch=expert_batch, fusion_weights=GOLDEN_WEIGHTS)
    print("通过：adapter 阶段未调用 open/path.open/np.load，不重新读取 prediction cache 或 oracle/TSF")

    assert_adapter_result(
        router_result,
        expert_batch=expert_batch,
        expected_weights_source="router_output_weights",
        atol=atol,
    )
    assert_adapter_result(
        explicit_result,
        expert_batch=expert_batch,
        expected_weights_source="explicit_fusion_weights",
        atol=atol,
    )
    ...
    print(
        ...
        f"hard_mae={router_result.summary['hard_mae']:.9f}，raw_soft_mae={router_result.summary['raw_soft_mae']:.9f}"
    )
    print("完成：Stage 1 EvaluationInput adapter smoke 全部通过")
