...
P2probe、P2b、P2c 和 P2d 均遵守 Round 1 pilot 协议：训练/选择只使用 `pilot_train` 和 `pilot_selection`，`diagnostic_balanced` 仅用于诊断，不使用 `pilot_test` 做模型选择。`p2d_final_test/` 中的 `pilot_test` 结果只用于冻结后的 final evaluation；其中记录的 final_test_only feature cache 可以复用，但不得用于模型、seed、epoch、variant 或超参数选择。
