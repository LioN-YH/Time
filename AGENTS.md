...
3. **上下文过长时强制 handoff**：当当前窗口上下文消耗接近 65% 左右、信息噪声明显增多，或继续推进会明显影响推理质量时，必须把 handoff 作为当前最高优先级步骤，立即暂停新增实现、长时间排查或大批量实验启动。先把当前目标、已完成步骤、正在运行/已停止的命令、失败点、关键路径、下一步命令和验证口径写入根目录 `HANDOFF.md`；如果该任务已有更合适的正式实验日志，也可以同步写入对应日志，但不能只在对话里口头说明。
4. **handoff 后再继续任务**：`HANDOFF.md` 写完后，应在当前窗口向用户明确说明已到达 handoff 条件、handoff 文件路径和建议继承方式，再通过新窗口、新线程、`/fork` 或等价方式继续。除非用户明确要求继续留在当前窗口，否则不要在 65% 阈值之后继续埋头推进主要任务。
5. **无法精确读数时保守触发**：如果当前界面或工具无法精确读取上下文占用比例，应根据长任务时长、对话长度、文件阅读量、工具输出量和推理噪声保守估计；一旦存在接近 65% 的明显风险，就提前执行 handoff，而不是等待自动压缩或接近上下文耗尽后再整理。
6. **长时间任务优先后台 launcher**：预计运行时间较长、可能跨窗口持续、或需要占用 GPU/CPU 多进程资源的实验与数据处理任务，应优先通过 launcher、`nohup`、`setsid`、`tmux` 或等价后台方式启动，而不是依赖当前交互式工具会话一直占用前台。启动前应确认输出目录、主日志、`status.json` 或等价状态文件、PID/进程组记录和可恢复/可重跑命令；启动后应做短间隔健康检查，并在实验日志或 `HANDOFF.md` 中记录监控命令、停止命令、输出路径和接手方式。
7. **可并行任务优先拆分**：如果发现一个目标可以拆成彼此独立或弱依赖的子任务，应优先拆分成多个 subagent 并行推进；主线程负责约束接口、合并结果和把控最终口径。
